...
class Point
{
    ...
    public void AddOneDistance(string dir)
    {
        if (dir == "R")
            x += 1;
        if (dir == "U")
...
class Wire
{
    ...
    public int GetClosestConnection(Point[] otherPoints)
    {
        int shortestConnectionDistance = 0;
        ...
        foreach (Point point in Points)
        {
            foreach (Point otherPoint in otherPoints)
            {
                if (point.x == otherPoint.x && point.y == otherPoint.y)
                {
                    ...
                    shortestConnectionDistance = (shortestConnectionDistance > 0 && shortestConnectionDistance < distance) ? shortestConnectionDistance : distance;
                }
            }
        }

        return shortestConnectionDistance;
    }

    public int GetShortestPathToConnection(Point[] otherPoints)
    {
        int shortestPath = -1;
        foreach (Point point in Points)
        {
            foreach (Point otherPoint in otherPoints)
            {
                if (point.x == otherPoint.x && point.y == otherPoint.y)
                {
                    int steps = point.steps + otherPoint.steps;
                    shortestPath = shortestPath > -1 && shortestPath < (point.steps + otherPoint.steps) ? shortestPath : steps;
                }
            }
        }
